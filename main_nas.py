...
def main(
    ...
    model.disable_alphas()
    model.disable_finetuning()
    model.enable_projectors()
    gradient_accumulation_steps = hps.get("gradient_accumulation_steps", 1)
    ...
    steps_per = [1,1] # Should be shared between mixed_dataset and mixed_opt
    steps_per = [int(s*gradient_accumulation_steps) for s in steps_per] # account for grad_accum_steps or else the alternation will be thrown off sync; there are basically grad_accum times the number of steps 
    
    mixed_dataset = MixedDataset(dataset_list=[train_dataset, eval_dataset], # has to be [train, eval] order, since model.get_mixed_... is in [model params, arch params] order
                                 batch_size=hps["per_device_train_batch_size"], #  sync batch_size
                                 steps_per_dataset=steps_per,)
    mixed_opt = model.get_mixed_optimizer(lr_model=0.001, betas_model=(0.9, 0.999), eps_model=1e-08, weight_decay_model=0.01, 
                                          lr_alphas=0.01, betas_alphas=(0.9, 0.999), eps_alphas=1e-08, weight_decay_alphas=0.01, # TODO; adjust these?
                                          steps_per_opt = steps_per)
    ...
    trainer = Trainer(
        ...
        eval_dataset=eval_dataset, # TODO?: eval on something else now that train & eval have been combined for darts/gaea?
        optimizers=(mixed_opt, None),
        callbacks=[WandbAlphasCallback(freq=1)],
        **trainer_kwargs,
    )
    print("before train")
    print(trainer.args.save_safetensors)
    trainer.train()
