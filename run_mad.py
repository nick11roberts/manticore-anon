...
def main(
    ...
    model = ManticoreForCausalLM(
        config=config, mamba=mamba, gptneo=gptneo
        # , based=based
    ).to(device=device)
    print(model)
    print(config)
    ...
    train_dataset = Dataset.from_dict({"input_ids": input_data, "label_ids": label})
    print(train_dataset)
    input_data_test = [item[0] for item in data["test"]]
    label_test = [item[1] for item in data["test"]]
    test_dataset = Dataset.from_dict(
        {"input_ids": input_data_test, "label_ids": label_test}
    )
