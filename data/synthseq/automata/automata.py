...
class AutomatonDataset(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, split):
        for i in itertools.count(start=0):
            ...
            yield i, {"input_ids": x, "label_ids": y}
...
class BinaryInputAutomaton(Automaton):
    ...
    def __init__(self, data_config):
        ...
        self.__info__ = (
            "  - prob1 (float in [0,1]): probability of token 1\n" + self.__info__
        )
...
class GridworldAutomaton(BinaryInputAutomaton):
    ...
    def __init__(self, data_config):
        ...
        self.__info__ = (
            f"1d Gridworld of n={self.n} states:\n"
    ...
    def f(self, x):
        ...
        if OLD_PY_VERSION:
            ...
            states = list(
                itertools.accumulate(x, lambda a, b: max(min(a + b, self.S), 0))
            )
            states = states[1:]
        else:
            states = list(
                itertools.accumulate(
                    x, lambda a, b: max(min(a + b, self.S), 0), initial=0
                )
            )
            states = states[
                1:
            ]  # remove the 1st entry with is the (meaningless) initial value 0
...
class ABABAutomaton(BinaryInputAutomaton):
    def __init__(self, data_config):
        ...
        self.__info__ = (
            "abab: an automaton with 4 states + 1 absorbing state:\n"
            + "- Inputs: binary strings\n"
            + "- Labels: depending on 'label_type'.\n"
            + "- Config:\n"
...
class AdderAutomaton(BinaryInputAutomaton):
    def __init__(self, data_config):
        ...
        self.addend_scales = np.array([2**i for i in range(self.n_addends)]).reshape(
            -1, 1
        )
        ...
        self.__info__ = (
            f"Adder of n={self.n_addends} binary numbers:\n"
...
class FlipFlopAutomaton(Automaton):
    def __init__(self, data_config):
        ...
        self.transition = np.array(
            ...
            + [[i + 1] * self.n_actions for i in range(self.n_states)]
        ).T

        self.__info__ = (
            f"Flipflop with n={self.n_states} states:\n"
...
class SymmetricAutomaton(PermutationAutomaton):
    ...
    def __init__(self, data_config):
        ...
        self.__info__ = (
            f"Symmetric group on n={self.n} objects:\n"
...
class AlternatingAutomaton(PermutationAutomaton):
    ...
    def __init__(self, data_config):
        ...
        self.__info__ = (
            f"Alternating group on n={self.n} objects:\n"
...
class CyclicAutomaton(Automaton):
    def __init__(self, data_config):
        ...
        self.__info__ = (
            "Cyclic group of n={self.n} states:\n"
...
class DihedralAutomaton(Automaton):
    def __init__(self, data_config):
        ...
        self.__info__ = (
            "Dihedral group of order 2n, where n={self.n}:\n"
    ...
    def f(self, x):
        ...
        if self.label_type == "state":
            labels = [
                self.get_state_label(each) for each in zip(toggle_status, positions)
            ]
...
class PermutationResetAutomaton(Automaton):
    def __init__(self, data_config):
        ...
        if type(self.generators[0]) is str:
            self.generators = [
                np.array(list(map(int, list(g)))) for g in self.generators
            ]
        ...
        self.n_actions = (
            self.n_states + self.n_generators
        )  # 1 reset symbol per state, 1 apply symbol per generator
    ...
    def sample(self):
        T = self.sample_length()
        x = (
            self.np_rng.choice(range(self.n_generators), p=self.perm_probs, size=T)
            + self.n_states
        )
