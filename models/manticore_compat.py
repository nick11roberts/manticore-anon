...
class GatedMixedOp(nn.Module):
    ...
    def forward(self, Wxs: List[torch.Tensor], xs: List[torch.Tensor]):
        ...
        return alphas[0] * (((1 - alphas[0]) * Wxs[0]) + (alphas[0] * xs[0])) + alphas[
            1
        ] * (((1 - alphas[1]) * Wxs[1]) + (alphas[1] * xs[1]))
...
class ManticoreForTask(PreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        self.mixtures = nn.ModuleList(
            [
                GatedMixedOp(k=2, alpha_mamba=self.config.alpha_mamba)
        ...
        self.gptneo_outs = nn.ModuleList(
            [
                (
                    projector_types[self.config.projector_type](
        ...
        # index_array_gptneo = np.load(f"{tokenizer_path}/index_array_1.npy")
        # index_array_mamba = np.load(f"{tokenizer_path}/index_array_2.npy")
        # vocab_size = np.max([np.max(index_array_gptneo), np.max(index_array_mamba)]) + 1
        # # vocab_size += 26  # Extra indices that Mamba has?
        # self.vocab_size = vocab_size
        self.gptneo_merged_emb = nn.Embedding(self.config.vocab_size, self.d_gptneo)
        self.mamba_merged_emb = nn.Embedding(self.config.vocab_size, self.d_mamba)
        nn.init.normal_(self.gptneo_merged_emb.weight, std=0.02)
        nn.init.normal_(self.mamba_merged_emb.weight, std=0.02)
        # nn.init.zeros_(self.gptneo_merged_emb.weight)
        # nn.init.zeros_(self.mamba_merged_emb.weight)

        # with torch.no_grad():
        #     self.gptneo_merged_emb.weight[index_array_mamba] = (
        #         self.mamba.model.backbone.embedding.weight[: len(index_array_mamba), :]
        #     )
        #     self.gptneo_merged_emb.weight[index_array_gptneo] = (
        #         self.gptneo.model.transformer.wte.weight[: len(index_array_gptneo), :]
        #     )

        #     self.mamba_merged_emb.weight[index_array_gptneo] = (
        #         self.gptneo.model.transformer.wte.weight[: len(index_array_gptneo), :]
        #     )
        #     self.mamba_merged_emb.weight[index_array_mamba] = (
        #         self.mamba.model.backbone.embedding.weight[: len(index_array_mamba), :]
        #     )

        # self.mamba_merged_emb.weight += self.gptneo_merged_emb.weight
        # self.mamba_merged_emb.weight /= 2

        # self.mamba_merged_emb.weight[-26:] = (
        #     self.mamba.model.backbone.embedding.weight[len(index_array_mamba) :, :]
        # )

        # print(index_array_gptneo)
        # print(index_array_mamba)
        # print(self.mamba_merged_emb.weight)
        # print(self.mamba_merged_emb.weight.shape)
        # quit()

        # print(len(index_array_mamba))
        # print(50280 - len(index_array_mamba))

        # print(self.gptneo.model.transformer.wte.weight.shape)
        # print(self.mamba.model.backbone.embedding.weight.shape)
        # print(self.gptneo_merged_emb.weight.shape)
        # quit()
    ...
    def forward_backbone(
        ...
        **kwargs,
        ...
        # if self.config.embedding_source == "gptneo":
        #     inputs_embeds = self.gptneo.model.transformer.wte(input_ids)
        # elif self.config.embedding_source == "mamba":
        #     inputs_embeds = self.mamba.model.backbone.embedding(input_ids)
        # else:
        ...
        inputs_embeds_gptneo = self.gptneo_merged_emb(input_ids)
        inputs_embeds_mamba = self.mamba_merged_emb(input_ids)
        hidden_states = (self.mixtures[0].get_alphas()[0] * inputs_embeds_mamba) + (
            self.mixtures[0].get_alphas()[1] * inputs_embeds_gptneo
        )
        # NOTE accessing alphas like this precludes the use of softmax. Switch to OMD
        ###########################################################
        ...
        for i in range(self.config.n_mixtures):
            ...
            # TODO gptneo ins
            # if i == 0:
            #     hidden_states = self.gptneo.model.transformer.wte(input_ids)
            #     print(self.gptneo.model.transformer.wte.weight.shape)
            hidden_states_gptneo = self.gptneo_ins[i](hidden_states)  # TODO also gate
            hidden_states_gptneo = (
                (1 - self.mixtures[i].get_alphas()[1]) * hidden_states_gptneo
            ) + (self.mixtures[i].get_alphas()[1] * hidden_states)
            _hidden_states_gptneo = self.gptneo.forward_intermediate(
                input_ids=None,
                hidden_states=hidden_states_gptneo,
                layer_indices=self.gptneo_inds[i],
                position_ids=position_ids,
            )
            hidden_states_gptneo = self.gptneo_outs[i](_hidden_states_gptneo)

            # print(_hidden_states_gptneo)
            # quit()
            ...
            #     print(self.mamba.model.backbone.embedding.weight.shape)
            #     quit()
            hidden_states_mamba = self.mamba_ins[i](hidden_states)  # TODO also gate
            hidden_states_mamba = (
                (1 - self.mixtures[i].get_alphas()[0]) * hidden_states_mamba
            ) + (self.mixtures[i].get_alphas()[0] * hidden_states)
            _hidden_states_mamba = self.mamba.forward_intermediate(
                input_ids=None,
            ...
            hidden_states_mamba = self.mamba_outs[i](_hidden_states_mamba)

            hidden_states = self.mixtures[i](
                [hidden_states_mamba, hidden_states_gptneo],
                [_hidden_states_mamba, _hidden_states_gptneo],
            )
            #################################################
        ...
        # elif self.config.embedding_source == "mamba":
        #     # hidden_states = self.mamba.model.backbone.norm_f(hidden_states)
        #     pass
        # else:
        #     raise NotImplementedError
...
class ManticoreForCausalLM(ManticoreForTask):
    def __init__(
        ...
        self.lm_head_gptneo = nn.Linear(
            self.d_gptneo, self.config.vocab_size, bias=False
        )
        self.lm_head_mamba = nn.Linear(self.d_mamba, self.config.vocab_size, bias=False)

        self.tie_weights()

    def tie_weights(self):
        self.lm_head_gptneo.weight = self.gptneo_merged_emb.weight
        self.lm_head_mamba.weight = self.mamba_merged_emb.weight
    ...
    def forward(
        ...
        **kwargs,
        ...
        lm_logits_gptneo = self.lm_head_gptneo(hidden_states)
        lm_logits_mamba = self.lm_head_mamba(hidden_states)

        # NOTE should we use the first or last mixture weights?
        lm_logits = (self.mixtures[-1].get_alphas()[0] * lm_logits_mamba) + (
            self.mixtures[-1].get_alphas()[1] * lm_logits_gptneo
        )
