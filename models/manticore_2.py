...
class ManticoreForTask(PreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        self.mixtures = nn.ModuleList(
            [
                GatedMixedOp(
                    k=len(self.models),
                    alpha_mamba=(None if m>=len(alpha_mambas) else alpha_mambas[m]),
    ...
    def load_alphas(self, supernet):
        assert len(supernet.mixtures) == len(self.mixtures)
        for mix, supernet_mix in zip(self.mixtures, supernet.mixtures):
            mix.alphas.data.copy_(supernet_mix.alphas.data)
        self.disable_alphas()
